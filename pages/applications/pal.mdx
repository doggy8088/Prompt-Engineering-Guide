...
[Gao等人，(2022)](https://arxiv.org/abs/2211.10435) 提出了一種使用LLMs來閱讀自然語言問題並產生程式作為中間推理步驟的方法。被稱為程式輔助語言模型(PAL)，它與思維鏈提示不同，因為它不是使用自由形式文本來獲取解決方案，而是將解決步驟解除安裝到程式設計執行時，例如Python直譯器。

<Screenshot src={PAL} alt="PAL" />
Image Source: [Gao et al., (2022)](https://arxiv.org/abs/2211.10435)

<Screenshot src={PAL} alt="PAL" />
圖片來源：[Gao et al.，(2022)](https://arxiv.org/abs/2211.10435)
...
具體而言，我們有興趣建立一個功能，允許使用LLM來回答需要日期理解的問題。我們將向LLM提供一個提示，其中包括一些範例，這些範例採用自[此處](https://github.com/reasoning-machines/pal/blob/main/pal/prompt/date_understanding_prompt.py)。
