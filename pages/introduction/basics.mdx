...
以上示範是LLMs今天所能實現的基本說明。今天的LLMs能夠執行各種高階任務，從文字摘要到數學推理到代碼生成。
...
當像上面那樣提示時，它也被稱為*零-shot提示*，即，您直接提示模型進行回應，而不需要任何有關您想要實現的任務的示範或示範。一些大型語言模型確實具有執行零-shot提示的能力，但這取決於手頭任務的複雜性和知識。
